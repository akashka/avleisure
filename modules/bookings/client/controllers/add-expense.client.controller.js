(function () {
	...
	function BookingsAddExpenseController($scope, $state, $window, booking, Authentication, Notification, BookingsService, EnquiriesService, UsersService, $timeout) {
		...
		$scope.deleteExpense = function (exp) {
			for (var e = 0; e < vm.booking.expenses.length; e++) {
				if (vm.booking.expenses[e].vendor_name == exp.vendor_name &&
					vm.booking.expenses[e].vendor_phone == exp.vendor_phone &&
					vm.booking.expenses[e].total_amount == exp.total_amount
				) {
					vm.booking.expenses.splice(e, 1);
				}
			}
		...
		$scope.expenseCleared = function(exp) {
			for (var e = 0; e < vm.booking.expenses.length; e++) {
				if (vm.booking.expenses[e].vendor_name == exp.vendor_name &&
					vm.booking.expenses[e].vendor_phone == exp.vendor_phone &&
					vm.booking.expenses[e].total_amount == exp.total_amount
				) {
					vm.booking.expenses[e].cleared = true;
				}
			}
